...
void setAngle(byte servoID, float angleDeg, int speed = ServoInitSpeed){
  if (angleDeg < 0) {
    angleDeg = 0;
  }
  if (angleDeg > ServoAngleRange) {
    angleDeg = ServoAngleRange;
  }
  int digitalPos = (int)((angleDeg * ServoDigitalRange) / ServoAngleRange);
  st.WritePosEx(servoID, digitalPos, speed, ServoInitACC);
}
...
void activeCtrl(int cmdInput){
  switch(cmdInput){
    ...
    case 30:setAngle(listID[activeNumInList], (float)server.arg(2).toFloat(), activeServoSpeed);break;
  }
}
